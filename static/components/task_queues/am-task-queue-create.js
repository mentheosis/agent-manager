  ...
  async loadProfiles() {
    try {
      ...
      const applyDefaults = () => {
        ...
        for (const [field, key, fallback] of [["max", "default_max_workers", 1], ["lease", "default_lease_secs", 60], ["seconds", "default_task_limit_secs", 3600], ["tokens", "default_task_limit_tokens", 2000000], ["rounds", "default_review_round_limit", 8]])
    ...
    try {
      const response = await fetch("/api/instances", {
        ...
        body: JSON.stringify({
          ...
          queue_review_round_limit: Number(this.querySelector("[name=rounds]").value),
