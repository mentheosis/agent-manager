...
class AmTaskQueueTasks extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.innerHTML = `<header class="queue-heading"><h2>Task queue</h2><nav><button data-view="tasks">Tasks</button><button data-view="activity">Activity</button></nav><p class="queue-error" role="alert"></p></header><section class="queue-task-list"><div class="queue-task-rows"></div><button class="queue-prev">Previous</button><button class="queue-next">Next</button></section><section class="loop-events" hidden></section>`;
  ...
  renderTasks(tasks) {
    ...
    if (!tasks.length) {
      rows.append(
        textElement(
          "p",
          "No tasks on this page. Producers enqueue tasks using the configured SQL protocol.",
        ),
      );
      return;
    }
